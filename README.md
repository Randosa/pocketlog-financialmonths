...
sets the `Secure` cookie flag accordingly — no extra configuration needed, as
long as the proxy sits on a private network (trusted by default).

`X-Forwarded-Proto` and `X-Real-IP`/`X-Forwarded-For` are only honoured when the
proxy's IP is trusted (`TRUSTED_PROXIES`). The default already trusts the
standard private/loopback ranges, so a proxy on a Docker bridge or LAN works out
of the box. Override `TRUSTED_PROXIES` only to narrow it down to a specific proxy
(e.g. `-e TRUSTED_PROXIES=172.17.0.5`), or set `-e TRUSTED_PROXIES=*` for a
single-proxy setup where the container port is not directly reachable from
untrusted networks.
