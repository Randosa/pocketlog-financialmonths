"""Client-side reload diagnostics.
...
"blink" to the user. The browser cannot write to the server log directly and
the page's memory dies with the reload, so the frontend parks a breadcrumb
in localStorage and delivers it here on its next authenticated boot
(core.js ``reportReloadEvents``). This endpoint's only job is to put those
events into the central log, where ``LOG_FILE`` makes them persistent.
...
def report_reload_events(payload: schemas.ReloadEventsIn, user: CurrentUser):
    for event in payload.events:
        logger.info(
            "client reload user_id=%d reason=%s occurred_at=%s",
            user.id,
            event.reason,
